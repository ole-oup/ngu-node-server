...
const thirtyMin = async (data) => {
  try {
    const dist1 = Number(data.cfg.thirtym.dist1);
    if (dist1 <= 0) throw 'dist1 too small';
    const dist2 = Number(data.cfg.thirtym.dist2);
    if (dist2 <= dist1) throw 'dist2 too small';
    const dist3 = Number(data.cfg.thirtym.dist3);
    if (dist3 <= dist2) throw 'dist3 too small';
    const dist4 = Number(data.cfg.thirtym.dist4);
    if (dist4 <= dist3) throw 'dist4 too small';
    const total = Number(data.cfg.thirtym.total);
